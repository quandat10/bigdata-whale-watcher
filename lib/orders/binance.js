...
const saveBook = () => {
  getPairs("binance").forEach((symbol) => {
    ...
    request(requestOptions)
      ...
      .catch((err) => {
        console.log("Binance Order book: ", err.message)
      })
...
const binance = (order) => {
  ...
  if (
    book[symbol] != undefined &&
    u >= book[symbol].lastUId + 1 &&
    ...
    try {
      if (book[symbol].bids.length > 5)
        outlier =
          parseFloat(book[symbol].bids[book[symbol].bids.length - 1][0]) -
          23 * sd(book[symbol].bids)
    ...
    bids.forEach((pricePoint) => {
      ...
      if (index != -1) {
        ...
        parseFloat(pricePoint[1]) != 0 &&
        parseFloat(pricePoint[0]) > outlier
      ) {
    ...
    try {
      if (book[symbol].asks.length > 5)
        outlier =
          parseFloat(book[symbol].asks[0][0]) + 23 * sd(book[symbol].asks)
      else throw new Error("")
    } catch (e) {
      // console.log(book[symbol]);
      let bid = book[symbol].bids[book[symbol].bids.length - 1]
      if (bid && bid.length > 0) {
        outlier = parseFloat(bid[0]) + 24 * sd(book[symbol].bids)
      }
    }
    ...
    let s_worth =
      s_total * askPrice * (usdExp.test(base) ? 1 : usd_values[base])

    let b_worth =
      b_total * askPrice * (usdExp.test(base) ? 1 : usd_values[base])

    let book_length_check =
      Math.abs(book[symbol].bids.length - book[symbol].asks.length) < 4
    if (
      book_length_check &&
      (s_worth > min_worth[currency] || b_worth > min_worth[currency])
    ) {
      ...
      if (sb_ratio > sensitivity && !wall[symbol].sell) {
        // || sb_ratio > wall[symbol].sell)) {
        ...
        if (alerts) message(messageObj)
      } else if (1 / sb_ratio > sensitivity && !wall[symbol].buy) {
        // || (1/sb_ratio) > wall[symbol].buy)) {
